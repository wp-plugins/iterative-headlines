...
function iterative_radio_goal_render(  ) { 
	...
	<p onclick='premiumClick(2);'><label class='disabled'><input type='radio' name='iterative_settings[headlines][goal]' <?php checked( $options['headlines']['goal'], 2 ); ?> disabled value='2'> I want my readers to stay longer on my site.</label> <small class='notyet'>Coming Soon</small></label></p>
	...
	<p onclick='premiumClick(8);'><label class='disabled'><input type='radio' name='iterative_settings[headlines][goal]' <?php checked( $options['headlines']['goal'], 8 ); ?> disabled value='8'> I have more advanced needs. <small class='notyet'>Coming Soon</small></label></p>
...
function iterative_options_page(  ) { 
	...
	<div class='wrap'>
		...
		<p class='description' title="Don't worry! If the optimization service goes offline, your headlines will continue to work fine."><strong>Optimization Service Status</strong>: <?php
			try { 
				$a = @file_get_contents("http://api.pathfinding.ca/");
				if($a === false) {
					echo "Unreachable. Don't worry though, your headlines will continue to work fine.";
				} else {
					if(floatval($a) < 5)
						echo "Healthy";
					else
						echo "Overloaded, but everything is working.";
				}
			} catch(Exception $e) {
				echo "Unknown";
			}
